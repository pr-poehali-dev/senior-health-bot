import { useState } from "react";
import Icon from "@/components/ui/icon";

const MEDICATIONS = [
  { expensive: "Нурофен", cheap: "Ибупрофен", category: "Обезболивающее/НПВС", economy: "до 85%" },
  { expensive: "Терафлю", cheap: "Парацетамол + аскорбиновая кислота", category: "ОРВИ/грипп", economy: "до 90%" },
  { expensive: "Эссенциале", cheap: "Фосфоглив", category: "Гепатопротектор", economy: "до 60%" },
  { expensive: "Де-нол", cheap: "Висмута трикалия дицитрат", category: "ЖКТ", economy: "до 75%" },
  { expensive: "Актовегин", cheap: "Церебролизин", category: "Ноотроп", economy: "до 50%" },
  { expensive: "Кларитин", cheap: "Лоратадин", category: "Антигистаминное", economy: "до 92%" },
  { expensive: "Мезим форте", cheap: "Панкреатин", category: "Ферменты/ЖКТ", economy: "до 80%" },
  { expensive: "Но-шпа", cheap: "Дротаверин", category: "Спазмолитик", economy: "до 90%" },
  { expensive: "Мирамистин", cheap: "Хлоргексидин", category: "Антисептик", economy: "до 95%" },
  { expensive: "Зиртек", cheap: "Цетиризин", category: "Антигистаминное", economy: "до 88%" },
  { expensive: "Диффлюкан", cheap: "Флуконазол", category: "Противогрибковое", economy: "до 95%" },
  { expensive: "Лазолван", cheap: "Амброксол", category: "Отхаркивающее", economy: "до 85%" },
  { expensive: "Ренни", cheap: "Кальция карбонат + магния карбонат", category: "Антацид/ЖКТ", economy: "до 70%" },
  { expensive: "Смекта", cheap: "Диосмектит", category: "Сорбент/ЖКТ", economy: "до 75%" },
  { expensive: "Називин", cheap: "Оксиметазолин", category: "Сосудосуживающее", economy: "до 80%" },
  { expensive: "Вольтарен", cheap: "Диклофенак", category: "НПВС/мазь", economy: "до 90%" },
  { expensive: "Омез", cheap: "Омепразол", category: "ЖКТ/антисекреторное", economy: "до 85%" },
  { expensive: "Линекс", cheap: "Бифидумбактерин", category: "Пробиотик", economy: "до 80%" },
  { expensive: "Найз", cheap: "Нимесулид", category: "Обезболивающее/НПВС", economy: "до 75%" },
  { expensive: "Аквамарис", cheap: "Натрия хлорид 0.9%", category: "Назальное средство", economy: "до 95%" },
  { expensive: "Трентал", cheap: "Пентоксифиллин", category: "Сосудистое", economy: "до 85%" },
  { expensive: "Детралекс", cheap: "Диосмин + гесперидин", category: "Венотоник", economy: "до 70%" },
  { expensive: "Кавинтон", cheap: "Винпоцетин", category: "Ноотроп/сосудистое", economy: "до 80%" },
  { expensive: "Бепантен", cheap: "Декспантенол", category: "Заживляющее", economy: "до 85%" },
  { expensive: "Фенистил", cheap: "Диметинден", category: "Антигистаминное/мазь", economy: "до 70%" },
  { expensive: "Имодиум", cheap: "Лоперамид", category: "Противодиарейное", economy: "до 90%" },
  { expensive: "Зовиракс", cheap: "Ацикловир", category: "Противовирусное", economy: "до 92%" },
  { expensive: "Гастал", cheap: "Алюминия гидроксид + магния гидроксид", category: "Антацид/ЖКТ", economy: "до 75%" },
  { expensive: "Амбробене", cheap: "Амброксол", category: "Отхаркивающее", economy: "до 80%" },
  { expensive: "Тизин", cheap: "Ксилометазолин", category: "Сосудосуживающее", economy: "до 85%" },
  { expensive: "Креон", cheap: "Панкреатин", category: "Ферменты/ЖКТ", economy: "до 75%" },
  { expensive: "Церукал", cheap: "Метоклопрамид", category: "Противорвотное", economy: "до 85%" },
  { expensive: "Супрастин", cheap: "Хлоропирамин", category: "Антигистаминное", economy: "до 70%" },
  { expensive: "Фастум гель", cheap: "Кетопрофен гель", category: "НПВС/мазь", economy: "до 80%" },
  { expensive: "Энтеросгель", cheap: "Полиметилсилоксана полигидрат", category: "Сорбент", economy: "до 60%" },
  { expensive: "Нексиум", cheap: "Эзомепразол", category: "ЖКТ/антисекреторное", economy: "до 85%" },
  { expensive: "Хилак форте", cheap: "Лактобактерии ацидофильные", category: "Пробиотик", economy: "до 65%" },
  { expensive: "Тантум верде", cheap: "Бензидамин", category: "Горло/НПВС", economy: "до 75%" },
  { expensive: "Отривин", cheap: "Ксилометазолин", category: "Сосудосуживающее", economy: "до 85%" },
  { expensive: "Пенталгин", cheap: "Парацетамол + дротаверин", category: "Обезболивающее", economy: "до 80%" },
  { expensive: "Канефрон", cheap: "Фитолизин", category: "Урология/фито", economy: "до 55%" },
  { expensive: "Аципол", cheap: "Лактобактерин", category: "Пробиотик", economy: "до 70%" },
  { expensive: "Мотилиум", cheap: "Домперидон", category: "ЖКТ/прокинетик", economy: "до 85%" },
  { expensive: "Гексорал", cheap: "Гексэтидин", category: "Горло/антисептик", economy: "до 75%" },
  { expensive: "Лиотон 1000", cheap: "Гепарин гель", category: "Венотоник/мазь", economy: "до 80%" },
  { expensive: "Снуп", cheap: "Ксилометазолин + морская вода", category: "Сосудосуживающее", economy: "до 70%" },
  { expensive: "Эспумизан", cheap: "Симетикон", category: "Ветрогонное/ЖКТ", economy: "до 75%" },
  { expensive: "Долобене", cheap: "Гепарин + декспантенол", category: "Противовоспалительное/мазь", economy: "до 65%" },
  { expensive: "Флюкостат", cheap: "Флуконазол", category: "Противогрибковое", economy: "до 90%" },
  { expensive: "Конкор", cheap: "Бисопролол", category: "Сердце/бета-блокатор", economy: "до 85%" },
  { expensive: "Престариум", cheap: "Периндоприл", category: "Сердце/от давления", economy: "до 80%" },
  { expensive: "Лориста", cheap: "Лозартан", category: "Сердце/от давления", economy: "до 75%" },
  { expensive: "Нолипрел", cheap: "Периндоприл + индапамид", category: "Сердце/от давления", economy: "до 70%" },
  { expensive: "Кардиомагнил", cheap: "Ацетилсалициловая кислота + магния гидроксид", category: "Сердце/антиагрегант", economy: "до 80%" },
  { expensive: "Плавикс", cheap: "Клопидогрел", category: "Сердце/антиагрегант", economy: "до 90%" },
  { expensive: "Крестор", cheap: "Розувастатин", category: "Сердце/холестерин", economy: "до 85%" },
  { expensive: "Аторис", cheap: "Аторвастатин", category: "Сердце/холестерин", economy: "до 80%" },
  { expensive: "Панангин", cheap: "Аспаркам", category: "Сердце/калий+магний", economy: "до 75%" },
  { expensive: "Верапамил СР", cheap: "Верапамил", category: "Сердце/антиаритмик", economy: "до 65%" },
  { expensive: "Беталок ЗОК", cheap: "Метопролол", category: "Сердце/бета-блокатор", economy: "до 85%" },
  { expensive: "Норваск", cheap: "Амлодипин", category: "Сердце/от давления", economy: "до 90%" },
  { expensive: "Диован", cheap: "Валсартан", category: "Сердце/от давления", economy: "до 80%" },
  { expensive: "Предуктал", cheap: "Триметазидин", category: "Сердце/метаболическое", economy: "до 75%" },
  { expensive: "Цистон", cheap: "Марены красильной экстракт", category: "Почки/фито", economy: "до 70%" },
  { expensive: "Канефрон Н", cheap: "Уролесан", category: "Почки/фито", economy: "до 55%" },
  { expensive: "Фитолизин паста", cheap: "Фитолизин Нефрокапс", category: "Почки/фито", economy: "до 50%" },
  { expensive: "Монурал", cheap: "Фосфомицин", category: "Почки/антибиотик", economy: "до 80%" },
  { expensive: "Нолицин", cheap: "Норфлоксацин", category: "Почки/антибиотик", economy: "до 75%" },
  { expensive: "Палин", cheap: "Пипемидовая кислота", category: "Почки/уроантисептик", economy: "до 65%" },
  { expensive: "5-НОК", cheap: "Нитроксолин", category: "Почки/уроантисептик", economy: "до 80%" },
  { expensive: "Гептрал", cheap: "Адеметионин", category: "Печень/гепатопротектор", economy: "до 60%" },
  { expensive: "Эссливер форте", cheap: "Фосфолипиды + витамины группы B", category: "Печень/гепатопротектор", economy: "до 65%" },
  { expensive: "Карсил", cheap: "Силимарин", category: "Печень/фито", economy: "до 70%" },
  { expensive: "Урсофальк", cheap: "Урсодезоксихолевая кислота", category: "Печень/желчегонное", economy: "до 75%" },
  { expensive: "Гепабене", cheap: "Расторопша + дымянка", category: "Печень/фито", economy: "до 60%" },
  { expensive: "Хофитол", cheap: "Артишока экстракт", category: "Печень/желчегонное", economy: "до 70%" },
  { expensive: "Овесол", cheap: "Овса экстракт + мята + бессмертник", category: "Печень/фито", economy: "до 55%" },
  { expensive: "Беродуал", cheap: "Ипратропия бромид + фенотерол", category: "Лёгкие/бронхолитик", economy: "до 60%" },
  { expensive: "Сальбутамол (Вентолин)", cheap: "Сальбутамол", category: "Лёгкие/бронхолитик", economy: "до 85%" },
  { expensive: "Серетид", cheap: "Сальметерол + флутиказон", category: "Лёгкие/астма", economy: "до 65%" },
  { expensive: "Пульмикорт", cheap: "Будесонид", category: "Лёгкие/ингаляции", economy: "до 70%" },
  { expensive: "Синекод", cheap: "Бутамират", category: "Лёгкие/от кашля", economy: "до 75%" },
  { expensive: "Либексин", cheap: "Преноксдиазин", category: "Лёгкие/от кашля", economy: "до 65%" },
  { expensive: "АЦЦ", cheap: "Ацетилцистеин", category: "Лёгкие/муколитик", economy: "до 80%" },
  { expensive: "Флуимуцил", cheap: "Ацетилцистеин", category: "Лёгкие/муколитик", economy: "до 75%" },
  { expensive: "Ринофлуимуцил", cheap: "Ацетилцистеин + туаминогептан", category: "Насморк/муколитик", economy: "до 55%" },
  { expensive: "Виброцил", cheap: "Фенилэфрин + диметинден", category: "Насморк/антигистаминное", economy: "до 60%" },
  { expensive: "Полидекса", cheap: "Неомицин + полимиксин + дексаметазон", category: "Насморк/антибиотик", economy: "до 50%" },
  { expensive: "Изофра", cheap: "Фрамицетин", category: "Насморк/антибиотик", economy: "до 55%" },
  { expensive: "Пиносол", cheap: "Масло эвкалипта + ментол + тимол", category: "Насморк/фито", economy: "до 65%" },
  { expensive: "Синупрет", cheap: "Бузина + первоцвет + щавель + вербена", category: "Насморк/фито", economy: "до 60%" },
  { expensive: "Африн", cheap: "Оксиметазолин", category: "Насморк/сосудосуживающее", economy: "до 80%" },
];

export default function MedicationsTab() {
  const [medSearch, setMedSearch] = useState("");

  const filteredMeds = MEDICATIONS.filter(m =>
    m.expensive.toLowerCase().includes(medSearch.toLowerCase()) ||
    m.cheap.toLowerCase().includes(medSearch.toLowerCase()) ||
    m.category.toLowerCase().includes(medSearch.toLowerCase())
  );

  return (
    <div className="animate-fade-in space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-foreground mb-1">Аналоги лекарств</h2>
        <p className="text-sm text-muted-foreground">Найдите доступные замены дорогих препаратов</p>
      </div>

      <div className="relative">
        <Icon name="Search" size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" fallback="Search" />
        <input
          value={medSearch}
          onChange={e => setMedSearch(e.target.value)}
          placeholder="Название препарата или категория..."
          className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-border bg-white text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition-all"
        />
      </div>

      <div className="glass-card rounded-xl p-3 flex items-center gap-2">
        <Icon name="Info" size={15} className="text-primary flex-shrink-0" fallback="Info" />
        <p className="text-xs text-muted-foreground">Замену препарата согласуйте с врачом или фармацевтом. Данные носят информационный характер.</p>
      </div>

      <div className="space-y-3">
        {filteredMeds.length === 0 && (
          <div className="text-center py-8 text-muted-foreground text-sm">Ничего не найдено</div>
        )}
        {filteredMeds.map((med, i) => (
          <div key={med.expensive} className="glass-card rounded-xl overflow-hidden card-hover" style={{ animationDelay: `${i * 50}ms` }}>
            <div className="flex items-stretch">
              <div className="w-1 bg-gradient-to-b from-primary to-accent flex-shrink-0" />
              <div className="p-4 flex-1">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide bg-muted px-2 py-0.5 rounded-full">{med.category}</span>
                  <span className="text-xs font-semibold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200">Экономия {med.economy}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex-1">
                    <div className="text-xs text-muted-foreground mb-0.5">Оригинал</div>
                    <div className="font-semibold text-foreground">{med.expensive}</div>
                  </div>
                  <div className="flex-shrink-0">
                    <Icon name="ArrowRight" size={16} className="text-accent" fallback="ArrowRight" />
                  </div>
                  <div className="flex-1 text-right">
                    <div className="text-xs text-muted-foreground mb-0.5">Аналог</div>
                    <div className="font-semibold text-primary">{med.cheap}</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}